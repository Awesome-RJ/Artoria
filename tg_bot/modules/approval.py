...
def approval(update, context):
    ...
    if target_user in approve_list:
        message.reply_text(
            "{} is an approved user. Auto Warns, antiflood, and blocklists won't apply to them.".format(
...
def approve(update, context):
    ...
    chat_id = str(chat.id)[1:]
    REDIS.sadd(
        f"approve_list_{chat_id}", mention_html(member.user.id, member.user.first_name)
    )
    message.reply_text(
        "{} has been approved in {}.".format(
            mention_html(member.user.id, member.user.first_name), chat.title
        ),
        parse_mode=ParseMode.HTML,
    )
...
def unapprove(update, context):
    ...
    message.reply_text(
        "{} is no longer approved in {}.".format(
            mention_html(member.user.id, member.user.first_name), chat.title
        ),
        parse_mode=ParseMode.HTML,
    )
...
def unapproveall(update, _):
    ...
    message.reply_text("Successully unapproved all users from {}.".format(chat.title))
...
APPROVED_HANDLER = DisableAbleCommandHandler(
    "approved", approved, filters=Filters.group
)
UNAPPROVE_ALL_HANDLER = DisableAbleCommandHandler(
    "unapproveall", unapproveall, filters=Filters.group
)
APPROVE_HANDLER = DisableAbleCommandHandler(
    "approve", approve, pass_args=True, filters=Filters.group
)
UNAPPROVE_HANDLER = DisableAbleCommandHandler(
    "unapprove", unapprove, pass_args=True, filters=Filters.group
)
