...
def user_join_fed(update, context):
    ...
    if is_user_fed_owner(fed_id, user.id) or user.id in SUDO_USERS:
        ...
        print(user_id)
        update.effective_message.reply_text(
            "Please get {} to confirm that they would like to be fed admin for dev testing fed.".format(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton(
                            text="Confirm", callback_data=f"fpromote_confirm={user_id}"
                        ),
                        InlineKeyboardButton(
                            text="Cancel", callback_data=f"fpromote_del={user_id}"
                        ),
                    ]
                ]
            ),
...
def fpro_callback(update, context):
    ...
    if query_match == "fpromote_confirm":
        if int(user_id) == query.from_user.id:
            ...
            if res:
                ...
                query.message.edit_text(
                    "User {} is now an admin of {} ({})".format(
                        mention_html(member.user.id, member.user.first_name),
                        fed_name,
                        fed_id,
                    ),
            ...
            context.bot.answer_callback_query(
                query.id, text="You're not the user being promoted!"
            )
        ...
        if int(user_id) == query.from_user.id:
            ...
            context.bot.answer_callback_query(query.id, text="Cancelled!")
        else:
            context.bot.answer_callback_query(
                query.id, text="You don't have enough permissions to cancel this."
            )
